...
const UserHead = () => {
  ...
  // Close suggestions on outside click (kept, even if you’re not using suggestions here)
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (suggestionRef.current && !suggestionRef.current.contains(event.target)) {
        // if later you add suggestion dropdown, handle it here
      }
    };
  ...
  // Show ticket detail modal + load comments
  ...
  const handleApprove = async () => {
    try {
      ...
      const payload = {
        ...
        ApproverId: user?.id == 881 ? 0 : user?.id,
        Approve: true,
        PAVAN_USER_ID: user?.id == 881 ? user?.id : 0,
      };
  ...
  const handleReject = async () => {
    try {
      ...
      if (!data) {
        setErrorMessage("Ticket rejection failed.");
      } else {
                  ...
                  <tbody>
                    {raisedData.map((ticket, index) => (
                      <tr key={ticket?.Id || index} className="table-row-hover">
                        ...
                        <td>{ticket?.Created_date ? new Date(ticket.Created_date).toLocaleString() : "-"}</td>
