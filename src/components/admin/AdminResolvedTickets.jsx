...
import { faEye, faUser } from "@fortawesome/free-solid-svg-icons"; // faUser imported but not used
import $ from "jquery";

import "datatables.net-dt";
import "datatables.net-dt/css/dataTables.dataTables.min.css";

const AdminResolvedTickets = ({ user }) => {
  const [errorMessage, setErrorMessage] = useState("");
  const [resolvedData, setResolvedData] = useState([]);
  const [loading, setLoading] = useState(false);
  const [showModal, setShowModal] = useState(false); // Renamed 'show' to 'showModal' for consistency
  ...
  // Fetch resolved tickets on component mount
  useEffect(() => {
    const fetchResolvedTickets = async () => {
      try {
        setLoading(true);
        const res = await resolvedTicketFunction();
        if (!res?.result) {
          setErrorMessage("No ticket data found.");
          setResolvedData([]); // Ensure resolvedData is empty if no results
        } else {
          setResolvedData(res.result);
        }
      } catch (err) {
        setErrorMessage("API error: " + err.message);
        setResolvedData([]); // Ensure resolvedData is empty on error
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    const tableId = "#adminResolvedTicketTable";
    ...
    // Cleanup function: destroy DataTable when component unmounts or resolvedData changes
    return () => {
      if (dataTableInstance.current) {
        dataTableInstance.current.destroy();
        dataTableInstance.current = null;
  ...
  }, [resolvedData]); // Re-run effect when resolvedData changes
  ...
  const fetchComments = async (token) => {
    ...
    try {
      ...
      const response = await getCommentFunction({ Token: token }); // Ensure this matches your API's expected payload
